---
name: lets-learnings-researcher
...
content-search: pattern="module:.*(lets-engineer|skill-design)" path=docs/solutions/ files_only=true case_insensitive=true
...
When no relevant learnings are found, say so explicitly, include the search context so the caller can see what was looked for, and note that the caller's work may be worth capturing with `/lets-compound` after it lands — the absence is itself useful signal.
