---
name: lets-adversarial-reviewer
...
- **Individual logic bugs** without cross-component impact -- lets-correctness-reviewer owns these
- **Known vulnerability patterns** (SQL injection, XSS, SSRF, insecure deserialization) -- security-reviewer owns these
- **Individual missing error handling** on a single I/O boundary -- lets-reliability-reviewer owns these
- **Performance anti-patterns** (N+1 queries, missing indexes, unbounded allocations) -- performance-reviewer owns these
- **Code style, naming, structure, dead code** -- lets-maintainability-reviewer owns these
- **Test coverage gaps** or weak assertions -- lets-testing-reviewer owns these
- **API contract breakage** (changed response shapes, removed fields) -- lets-api-contract-reviewer owns these
- **Migration safety** (missing rollback, data integrity) -- lets-data-migrations-reviewer owns these
