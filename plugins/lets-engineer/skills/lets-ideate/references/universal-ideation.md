...
- **Brainstorm a selected idea** — go deeper on one idea through dialogue. Unlike repo mode, this is not the first step of an implementation chain — there is no `lets-plan` → `lets-work` after; `lets-brainstorm` in universal mode develops the idea further (e.g., expands a name into a brand brief, a plot into an outline, a decision into a weighed framework) and ends there. Persist first per the §6.2 contract in `references/post-ideation-workflow.md`: save the survivors to a local markdown file (the current working directory by default, or `docs/ideation/` when inside a git repo), mark the chosen idea as `Explored`, then load `lets-brainstorm` with that idea as the seed.
- **Save and end** — write the survivors to a local markdown file and end. Default to the current working directory, or `docs/ideation/` when inside a git repo; honor an explicit custom path. If the write fails, offer a custom path or keep the ideation in conversation without saving.
