...
|-------|---------------------------|
| `lets-security-reviewer` | Auth, public endpoints, user input, permissions |
| `lets-performance-reviewer` | DB queries, data transforms, caching, async |
| `lets-api-contract-reviewer` | Routes, serializers, type signatures, versioning |
| `lets-data-migrations-reviewer` | Migrations, schema changes, backfills |
| `lets-reliability-reviewer` | Error handling, retries, timeouts, background jobs |
| `lets-adversarial-reviewer` | Diff >=50 changed non-test/non-generated/non-lockfile lines, or auth, payments, data mutations, external APIs |
| `lets-previous-comments-reviewer` | Reviewing a PR that has existing review comments or threads |

**Stack-specific conditional (selected per diff):**

| Agent | Select when diff touches... |
|-------|---------------------------|
| `lets-dhh-rails-reviewer` | Rails architecture, service objects, session/auth choices, or Hotwire-vs-SPA boundaries |
| `lets-kieran-rails-reviewer` | Rails application code where conventions, naming, and maintainability are in play |
| `lets-kieran-python-reviewer` | Python modules, endpoints, scripts, or services |
| `lets-kieran-typescript-reviewer` | TypeScript components, services, hooks, utilities, or shared types |
| `lets-julik-frontend-races-reviewer` | Stimulus/Turbo controllers, DOM events, timers, animations, or async UI flows |
| `lets-swift-ios-reviewer` | Swift files, SwiftUI views, UIKit controllers, entitlements, privacy manifests, Core Data models, SPM manifests, storyboards/XIBs, or semantic build-setting/target/signing changes in .pbxproj |

**CE conditional (migration-specific):**

| Agent | Select when diff includes migration files |
|-------|------------------------------------------|
| `lets-schema-drift-detector` | Cross-references schema.rb against included migrations |
| `lets-deployment-verification-agent` | Produces deployment checklist with SQL verification queries |
...
The following paths are lets-engineer pipeline artifacts and must never be flagged for deletion, removal, or gitignore by any reviewer:

- `docs/brainstorms/*` -- requirements documents created by lets-brainstorm
- `docs/plans/*.md` -- plan files created by lets-plan (decision artifacts; execution progress is derived from git, not stored in plan bodies)
...
Use the repository portion of the returned PR URL as `<base-repo>` (for example, `williamzelesny/lets-engineer` from `https://github.com/williamzelesny/lets-engineer/pull/348`).
...
- lets-agent-native-reviewer (always)
- lets-learnings-researcher (always)
...
- lets-schema-drift-detector -- migration files present
```
...
**CE always-on agents** (lets-agent-native-reviewer, lets-learnings-researcher) are dispatched as standard Agent calls through the same bounded parallel scheduler as the persona agents. Give them the same review context bundle the personas receive: entry mode, any PR metadata gathered in Stage 1, intent summary, review base branch name when known, `BASE:` marker, file list, diff, and `UNTRACKED:` scope notes. Do not invoke them with a generic "review this" prompt. Their output is unstructured and synthesized separately in Stage 6.

**CE conditional agents** (lets-schema-drift-detector, lets-deployment-verification-agent) are also dispatched as standard Agent calls through the same bounded parallel scheduler when applicable. Pass the same review context bundle plus the applicability reason (for example, which migration files triggered the agent). For lets-schema-drift-detector specifically, pass the resolved review base branch explicitly so it never assumes `main`. Their output is unstructured and must be preserved for Stage 6 synthesis just like the CE always-on agents.
...
7. **Learnings & Past Solutions.** Surface lets-learnings-researcher results: if past solutions are relevant, flag them as "Known Pattern" with links to docs/solutions/ files.
8. **Agent-Native Gaps.** Surface lets-agent-native-reviewer results. Omit section if no gaps found.
9. **Schema Drift Check.** If lets-schema-drift-detector ran, summarize whether drift was found. If drift exists, list the unrelated schema objects and the required cleanup command. If clean, say so briefly.
10. **Deployment Notes.** If lets-deployment-verification-agent ran, surface the key Go/No-Go items: blocking pre-deploy checks, the most important verification queries, rollback caveats, and monitoring focus areas. Keep the checklist actionable rather than dropping it into Coverage.
...
- Also write `metadata.json` alongside the findings so downstream skills (e.g., `lets-polish-beta`) can verify the artifact matches the current branch and HEAD. Minimum fields:
  ```json
  {
    "run_id": "<run-id>",
