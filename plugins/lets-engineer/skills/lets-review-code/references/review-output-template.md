...
- **Learnings & Past Solutions section** -- results from lets-learnings-researcher, with links to docs/solutions/ files
- **Agent-Native Gaps section** -- results from lets-agent-native-reviewer. Omit if no gaps found.
- **Schema Drift Check section** -- results from lets-schema-drift-detector. Omit if the agent did not run.
- **Deployment Notes section** -- key checklist items from lets-deployment-verification-agent. Omit if the agent did not run.
