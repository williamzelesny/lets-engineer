...
1. **Anchor, not plan.** Strategy is what the product is and why. Features belong in `lets-brainstorm`; schedules belong in the issue tracker. Do not let either creep into the doc.
...
After writing, note in one line where the file lives and that `lets-ideate`, `lets-brainstorm`, and `lets-plan` will pick it up as grounding on their next run.

If no downstream skill has run yet on this repo, suggest `lets-ideate` or `lets-brainstorm` skills as a next step.
...
- Does not write product requirements or implementation plans - those are `lets-brainstorm` and `lets-plan`.
