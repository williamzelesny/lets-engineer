...
- The finding's `section` is framing-level (Problem Frame, Summary, Overview, Why, Motivation, Goals — `Summary` is the new lets-plan / lets-brainstorm template heading; `Overview` retained as legacy) OR the finding explicitly questions whether a named component should exist
...
- Requirements document → `lets-plan`
- Plan document → `lets-work`
