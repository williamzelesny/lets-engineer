...
This template is used by the lets-review-docs orchestrator to spawn each reviewer subagent. Variable substitution slots are filled at dispatch time.
...
- You are a leaf reviewer inside an already-running lets-engineer review workflow. Do not invoke lets-engineer skills or agents unless this template explicitly instructs you to. Perform your analysis directly and return findings in the required output format only.
