...
If `agent-browser` is not installed, inform the user: "`agent-browser` is not installed. Run `/lets-setup` to install required dependencies." Then fall back to a lower tier (static screenshots or skip).
