...
If `agent-browser` is not installed, inform the user: "`agent-browser` is not installed. Run `/lets-setup` to install required dependencies." Then skip to the CLI or fallback sections below.
