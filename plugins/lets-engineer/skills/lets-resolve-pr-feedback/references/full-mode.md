...
**For review threads** (`review_threads`): Spawn a `lets-pr-comment-resolver` agent for each new thread that is NOT already assigned to a cluster from step 3. Clustered threads are handled by cluster dispatch below -- do not dispatch them individually.
...
**For PR comments and review bodies** (`pr_comments`, `review_bodies`): These lack file/line context. Spawn a `lets-pr-comment-resolver` agent for each actionable non-clustered item. The agent receives the comment ID, body text, PR number, and feedback type (`pr_comment` or `review_body`). The agent must identify the relevant files from the comment text and the PR diff.

### Cluster dispatch

For each cluster identified in step 3, dispatch ONE `lets-pr-comment-resolver` agent that receives:
...
1. Stage only files reported by subagents and commit with a message referencing the PR:
