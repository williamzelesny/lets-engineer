# `/lets-optimize` Usage Guide

## What This Skill Is For

`/lets-optimize` is for hard engineering problems where:
...
Use `/lets-optimize` when the problem looks like:
...
`/lets-optimize` is usually the wrong tool when:
...
```text
Use /lets-optimize to find the smallest memory setting that keeps this service stable under our load test.
...
```text
Use /lets-optimize to improve issue and PR clustering quality.
...
```text
Use /lets-optimize to create a summarization prompt for issues and PRs that minimizes token spend while still producing summaries that are good enough for downstream clustering.
