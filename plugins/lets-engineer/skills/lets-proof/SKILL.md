---
name: lets-proof
description: Run human-in-the-loop review loops over markdown via Proof (proofeditor.ai) — share, view, comment on, edit, and sync collaborative docs. Use when the user says "view this in proof", "share to proof", "HITL this doc", or wants a shared markdown review surface for a spec, plan, or draft, including handoffs from lets-brainstorm, lets-ideate, or lets-plan. Do not trigger on "proof" meaning evidence, math proofs, proof-of-concept, or "proofread this".
allowed-tools:
  - Bash
  - Read
  - Write
...
- **Machine ID (`by` on every op, `X-Agent-Id` header):** `ai:lets-engineer` — stable, lowercase-hyphenated, machine-parseable. Appears in marks, events, and the API response.
- **Display name (`name` on `POST /presence`):** `lets-engineer` — human-readable, shown in Proof's presence chips and comment-author badges.
...
- **Upstream skill handoff** — `lets-brainstorm`, `lets-ideate`, or `lets-plan` finishes a draft and hands it off for human review before the next phase, passing the file path and title explicitly.
...
- Header: `X-Agent-Id: ai:lets-engineer` (required for presence; include on ops for consistent attribution)
...
**Comment on text:**
```json
{"type": "comment.add", "quote": "text to comment on", "by": "ai:lets-engineer", "text": "Your comment here", "baseToken": "<token>"}
```

**Reply to a comment:**
```json
{"type": "comment.reply", "markId": "<id>", "by": "ai:lets-engineer", "text": "Reply text", "baseToken": "<token>"}
```

**Reply and resolve in one mutation:**
```json
{"type": "comment.reply", "markId": "<id>", "by": "ai:lets-engineer", "text": "Fixed.", "resolve": true, "baseToken": "<token>"}
```

**Batch existing-thread comment mutations:**
```json
{"by": "ai:lets-engineer", "baseToken": "<token>", "operations": [
...
```json
{"type": "comment.resolve", "markId": "<id>", "by": "ai:lets-engineer", "baseToken": "<token>"}
{"type": "comment.unresolve", "markId": "<id>", "by": "ai:lets-engineer", "baseToken": "<token>"}
```

**Suggest a replacement (pending — user must accept/reject):**
```json
{"type": "suggestion.add", "kind": "replace", "quote": "original text", "by": "ai:lets-engineer", "content": "replacement text", "baseToken": "<token>"}
```

**Suggest and immediately apply (tracked but committed — user can reject to revert):**
```json
{"type": "suggestion.add", "kind": "replace", "quote": "original text", "by": "ai:lets-engineer", "content": "replacement text", "status": "accepted", "baseToken": "<token>"}
```
...
```json
{"type": "suggestion.accept", "markId": "<id>", "by": "ai:lets-engineer", "baseToken": "<token>"}
{"type": "suggestion.reject", "markId": "<id>", "by": "ai:lets-engineer", "baseToken": "<token>"}
```
...
```json
{"type": "rewrite.apply", "content": "full new markdown", "by": "ai:lets-engineer", "baseToken": "<token>"}
```
...
**Required headers:**
- `X-Agent-Id: ai:lets-engineer` (identity for presence; keep aligned with `by`)
...
| POST | `/marks/suggest-replace` | `{"quote":"old","by":"ai:lets-engineer","content":"new"}` |
| POST | `/marks/suggest-insert` | `{"quote":"after this","by":"ai:lets-engineer","content":"insert"}` |
| POST | `/marks/suggest-delete` | `{"quote":"delete this","by":"ai:lets-engineer"}` |
| POST | `/marks/comment` | `{"quote":"text","by":"ai:lets-engineer","text":"comment"}` |
| POST | `/marks/reply` | `{"markId":"<id>","by":"ai:lets-engineer","text":"reply"}` |
| POST | `/marks/resolve` | `{"markId":"<id>","by":"ai:lets-engineer"}` |
| POST | `/marks/accept` | `{"markId":"<id>"}` |
| POST | `/marks/reject` | `{"markId":"<id>"}` |
| POST | `/rewrite` | Last-resort whole-doc replacement: `{"content":"full markdown","by":"ai:lets-engineer"}` |
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -d "$(jq -n --arg base "$BASE" '{type:"comment.add",quote:"text",by:"ai:lets-engineer",text:"comment",baseToken:$base}')")
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -H "X-Agent-Id: ai:lets-engineer" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$BASE" '{type:"suggestion.add",kind:"replace",quote:"old",by:"ai:lets-engineer",content:"new",baseToken:$base}')")
NEXT_BASE=$(printf '%s' "$OP_RESP" | jq -r '.mutationBase.token // empty')
[ -n "$NEXT_BASE" ] && BASE="$NEXT_BASE"
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -H "X-Agent-Id: ai:lets-engineer" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$BASE" '{type:"suggestion.add",kind:"replace",quote:"old",by:"ai:lets-engineer",content:"new",status:"accepted",baseToken:$base}')")
NEXT_BASE=$(printf '%s' "$OP_RESP" | jq -r '.mutationBase.token // empty')
[ -n "$NEXT_BASE" ] && BASE="$NEXT_BASE"
...
curl -X POST "https://www.proofeditor.ai/api/agent/abc123/edit/v2?return=minimal" \
  ...
  -H "X-Agent-Id: ai:lets-engineer" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$EDIT_BASE" '{by:"ai:lets-engineer",baseToken:$base,operations:[{op:"find_replace_in_doc",find:"old",replace:"new",occurrence:"all"}]}')"
```
...
curl -s -X POST "https://www.proofeditor.ai/api/agent/$SLUG/presence" \
  ...
  -H "x-share-token: $TOKEN" \
  -H "X-Agent-Id: ai:lets-engineer" \
  -d '{"name":"lets-engineer","status":"reading","summary":"Uploaded doc"}'
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/$SLUG/ops" \
  ...
  -H "X-Agent-Id: ai:lets-engineer" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$BASE" '{type:"comment.add",quote:"Content here",by:"ai:lets-engineer",text:"Added a note",baseToken:$base}')")
NEXT_BASE=$(printf '%s' "$OP_RESP" | jq -r '.mutationBase.token // empty')
[ -n "$NEXT_BASE" ] && BASE="$NEXT_BASE"
...
curl -X POST "https://www.proofeditor.ai/api/agent/$SLUG/edit/v2?return=minimal" \
  ...
  -H "X-Agent-Id: ai:lets-engineer" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$EDIT_BASE" '{by:"ai:lets-engineer",baseToken:$base,operations:[{op:"find_replace_in_doc",find:"Content",replace:"Updated content",occurrence:"all"}]}')"
```
...
- Always include `by: "ai:lets-engineer"` on every op and `X-Agent-Id: ai:lets-engineer` in headers for consistent attribution
