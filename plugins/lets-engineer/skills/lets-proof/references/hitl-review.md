...
Human-in-the-loop iteration loop for a markdown document shared via Proof. Invoked either by an upstream skill (`lets-brainstorm`, `lets-ideate`, `lets-plan`) handing off a draft it produced, or directly by the user asking to iterate on an existing markdown file they already have on disk ("share this to proof and iterate", "HITL this doc with me"). Mechanics are identical in both cases: upload the local doc, let the user annotate in Proof's web UI, ingest feedback as in-thread replies and agreed edits, and sync the final doc back to disk.
...
- **Recommended next step** (optional, caller-specific): short string the caller wants echoed in the final terminal output (e.g., "Recommended next: `/lets-plan`"). Not used on direct-user invocation — the terminal report simply summarizes the iteration and asks what's next.

Agent identity is fixed, not a parameter: every API call uses agent ID `ai:lets-engineer` and display name `lets-engineer`. Callers do not override this.
...
3. `POST /api/agent/{slug}/presence` with `X-Agent-Id: ai:lets-engineer`, `x-share-token: <token>`, body `{"name":"lets-engineer","status":"reading","summary":"Uploaded doc for review"}`
...
```json
{"type":"suggestion.add","kind":"replace","quote":"<anchor>","content":"<new>","by":"ai:lets-engineer","status":"accepted","baseToken":"<token>"}
```
...
curl -X POST "https://www.proofeditor.ai/api/agent/{slug}/edit/v2" \
  ...
  -H "X-Agent-Id: ai:lets-engineer" -H "Idempotency-Key: <uuid>" \
  -d '{"by":"ai:lets-engineer","baseToken":"<token>","operations":[...]}'
```
...
- Set `by: "ai:lets-engineer"` and header `X-Agent-Id: ai:lets-engineer`.
...
LANDED=$(printf '%s' "$STATE" | jq --arg q "X" --arg t "Y" \
  '[.marks[]? | select(.by == "ai:lets-engineer" and .quote == $q and (.thread[0].text // .text) == $t)] | length')
if [ "$LANDED" -gt 0 ]; then
...
- `by: "ai:lets-engineer"` in the request body
- `X-Agent-Id: ai:lets-engineer` in headers (required for presence; recommended for ops for consistent attribution)

Display name `lets-engineer` is bound via `POST /presence` with `{"name":"lets-engineer", ...}`. Set this once after upload; it carries across subsequent ops.
