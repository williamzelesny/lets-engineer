...
- Bad: "Adds `evidence-decider.ts`, modifies `lets-commit-push-pr/SKILL.md` to call it, and updates two test files."
...
In order: opening → body sections that earn their keep → test plan if non-obvious → evidence block if one exists → lets-engineer badge after a `---` rule.
...
[![lets-engineer](https://img.shields.io/badge/Built_with-lets--engineer-6366f1)](https://github.com/williamzelesny/lets-engineer)
