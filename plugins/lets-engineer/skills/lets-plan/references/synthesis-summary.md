...
- AND Phase 0.4 stayed in lets-plan (did not route to lets-debug, lets-work, or universal-planning)
...
- **Solo variant**: common redirects include "this is bigger than I thought — let me brainstorm first" (suggest `/lets-brainstorm`), "this is just a fix, no plan needed" (suggest `/lets-work`), or "I need to investigate first" (suggest `/lets-debug`).
- **Brainstorm-sourced variant**: less common, but possible — "actually this scope is wrong, take it back to brainstorm" (suggest `/lets-brainstorm` to revise the upstream doc).

In either case: stop lets-plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
