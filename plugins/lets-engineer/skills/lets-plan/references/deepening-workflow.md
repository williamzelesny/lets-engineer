...
- `lets-spec-flow-analyzer` for missing user flows, edge cases, and handoff gaps
- `lets-repo-research-analyst` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `lets-learnings-researcher` for institutional knowledge and past solved problems
- `lets-framework-docs-researcher` for official framework or library behavior
- `lets-best-practices-researcher` for current external patterns and industry guidance
- Add `lets-git-history-analyzer` only when historical rationale or prior art is materially missing

**Key Technical Decisions**
- `lets-architecture-strategist` for design integrity, boundaries, and architectural tradeoffs
- Add `lets-framework-docs-researcher` or `lets-best-practices-researcher` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `lets-architecture-strategist` for validating that the technical design accurately represents the intended approach and identifying gaps
- `lets-repo-research-analyst` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `lets-best-practices-researcher` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `lets-repo-research-analyst` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues
- `lets-pattern-recognition-specialist` for consistency, duplication risks, and alignment with existing patterns
- Add `lets-spec-flow-analyzer` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `lets-architecture-strategist` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `lets-performance-oracle` for scalability, latency, throughput, and resource-risk analysis
  - `lets-security-sentinel` for auth, validation, exploit surfaces, and security boundary review
  - `lets-data-integrity-guardian` for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `lets-security-sentinel` for security, auth, privacy, and exploit risk
  - `lets-data-integrity-guardian` for persistent data safety, constraints, and transaction boundaries
  - `lets-data-migration-expert` for migration realism, backfills, and production data transformation risk
  - `lets-deployment-verification-agent` for rollout checklists, rollback planning, and launch verification
  - `lets-performance-oracle` for capacity, latency, and scaling concerns
...
1. **Summarize the agent and its target section** — e.g., "The lets-architecture-strategist reviewed Key Technical Decisions and found:"
...
- Reorder or split implementation units when sequencing is weak — but **never renumber existing U-IDs**. Reordering preserves U-IDs in their new order (e.g., U1, U3, U5 reordered is correct; renumbering to U1, U2, U3 is not). Splitting keeps the original U-ID on the original concept and assigns the next unused number to the new unit. Renumbering breaks lets-work blocker and verification references that were written against the original IDs
...
- Recommend `lets-brainstorm` if the gap is truly product-defining
