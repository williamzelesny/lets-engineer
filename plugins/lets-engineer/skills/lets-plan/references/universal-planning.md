...
This file is loaded when lets-plan detects a non-software task (Phase 0.1b). It replaces the software-specific phases (0.2 through 5.1) with a domain-agnostic planning workflow.
...
- **Pipeline mode?** If invoked from LFG or any `disable-model-invocation` context: output "This is a non-software task. The LFG pipeline requires lets-work, which only supports software tasks. Use `/lets-plan` directly for non-software planning." and stop.

Once past these checks, commit to producing a plan. Do not exit because the task looks like a "lookup" or "research question" — the user invoked `lets-plan` because they want a structured output.
...
When research is recommended, do it — don't just offer. Stale recommendations (closed restaurants, rethemed attractions, outdated prices) are worse than no recommendations. The user invoked `/lets-plan` because they want a good plan, not a disclaimer about training data.
...
**Question:** "Plan ready. Where should I save it?"
...
Write the plan as a markdown file using the filename convention `YYYY-MM-DD-<descriptive-name>-plan.md`. Start the document with a `# Title` heading, followed by `Created: YYYY-MM-DD` on the next line. No YAML frontmatter.

Do not offer `/lets-work` (software-only) or issue creation (not applicable to non-software plans).
