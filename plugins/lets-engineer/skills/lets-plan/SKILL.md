...
- **Open in Proof (web app) — review and comment to iterate with the agent** — Load the `lets-proof` skill in HITL-review mode with the plan file as `source file`, the plan title as `doc title`, identity `ai:lets-engineer` / `lets-engineer`, and recommended next step `/lets-work`. Then follow the post-HITL resync logic in `references/plan-handoff.md`, which handles the four `lets-proof` return statuses, re-runs `lets-doc-review` after material edits, and falls back gracefully on upload failure.
